...
/// 立方体を描画する関数
/// </summary>
/// <param name="aabb">立方体構造体</param>
/// <param name="viewProjectionMatrix">射影行列</param>
/// <param name="viewPortMatrix">ビューポート行列</param>
/// <param name="color">立方体の色</param>
void MyDebug::DrawAABB(const AABB& aabb, const Matrix4x4& viewProjectionMatrix, const Matrix4x4& viewPortMatrix, uint32_t color) {
	...
	for (int i = 0; i < 8; i++) {
		// スクリーン座標系に変換
		aabbWorldVertices[i] = MyMath::Transform(aabbVertices[i], viewProjectionMatrix);
		aabbScreenVertices[i] = MyMath::Transform(aabbWorldVertices[i], viewPortMatrix);
	}
